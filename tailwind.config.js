/** Tailwind の設定。
 *  もとは index.html のインライン <script> に書かれ、cdn.tailwindcss.com が実行時に読んでいた。
 *  CDN はバージョン無指定・SRI 無しで、決済リンクを描くページで DOM 全権を持つため撤去し、
 *  ビルド時に CSS を生成する方式へ移した。値は移設時点の本番 index.html と一致させてある。
